...
export default async function handler(req, res) {
  ...
  try {
    ...
    const saved = await fetch(`${supabaseUrl}/rest/v1/profiles`, { method: "POST", headers: { ...headers(serviceKey), Prefer: "return=representation" }, body: JSON.stringify({ id: newUser.id, organization_id: profile.organization_id, full_name: name.trim(), role: "staff", phone: phone.trim(), is_active: Boolean(active) }) });
    if (!saved.ok) return json(res, 500, { error: "Agent login was created but its profile could not be saved" });
    return json(res, 201, { id: newUser.id, name: name.trim(), email: email.trim().toLowerCase() });
