-- Run after 002_finance_operations.sql.
...
begin
  ...
  update public.finance_accounts set collection_amount = account_collection_amount,
...
as $$
begin
  delete from public.finance_accounts where id = account_id
    and organization_id = public.current_organization_id();
  if not found then raise exception 'Finance account not found in this workspace'; end if;
end;
$$;

grant execute on function public.update_finance_account(uuid,text,text,text,numeric,numeric,numeric,numeric,numeric,numeric) to authenticated;
