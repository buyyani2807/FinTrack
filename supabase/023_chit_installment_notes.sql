-- Add notes to isolated Chit Fund installment payments.
alter table public.chit_installments add column if not exists notes text;
drop function if exists public.chit_update_installment_payment(uuid,numeric,date,public.payment_mode,text,numeric,numeric);
...
begin
  ...
  select net_amount_due into due from public.chit_installments where id = input_installment_id and organization_id = public.current_organization_id();
  ...
  update public.chit_installments set amount_paid=round(input_amount_paid,2), paid_date=case when input_amount_paid>0 then coalesce(input_paid_date,current_date) else null end,
    ...
    cash_amount=coalesce(input_cash_amount,0), upi_amount=coalesce(input_upi_amount,0), notes=nullif(trim(input_notes),''),
...
grant execute on function public.chit_update_installment_payment(uuid,numeric,date,public.payment_mode,text,numeric,numeric,text) to authenticated;
