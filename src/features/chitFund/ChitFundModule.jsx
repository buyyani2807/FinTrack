...
const Modal = ({ children }) => <div className="modal-bg" onClick={event => event.stopPropagation()}><div className="modal" onClick={event => event.stopPropagation()}>{children}</div></div>;
...
function ChitDeleteMemberControl({ token, scheme, enrollment, onDeleted }) {
...
function ChitSchemeHeaderActions({ token, scheme, memberCount, onAddMember, onSchemeDeleted, extra }) {
  return <div className="row">
    {extra}
    {scheme.status === "draft" && onAddMember && <Button onClick={onAddMember}>+ Add member</Button>}
...
const byMemberName = (a, b) => enrollmentName(a).localeCompare(enrollmentName(b), undefined, { sensitivity: "base" });
const paymentsByMemberName = (payments, enrollments, cycles = []) => [...(payments || [])].sort((a, b) => {
...
const paymentStatusClass = status => status === "paid" ? "completed" : status === "overdue" ? "overdue" : status === "partially paid" ? "active" : status === "waived" ? "closed" : "";
const ChitPaymentStatus = ({ row, asOfDate = today() }) => {
...
function FixedChitSchemeForm({ title, form, setForm, busy, error, onClose, onSubmit, submitLabel }) {
  const set = (key, value) => setForm(current => ({ ...current, [key]: value }));
  const commissionRupees = fixedCommissionFromPercent(form.chitValue, form.commissionPercent);
  const derivedInitial = Number(form.chitValue || 0) - commissionRupees;
  return <Modal><h2 className="title">{title}</h2><form onSubmit={onSubmit}><div className="form spacer"><Field label="Scheme name *"><input required value={form.name} onChange={e => set("name", e.target.value)} /></Field><Field label="Chit type"><input value="Fixed" disabled /></Field><Field label="Start date *"><input required type="date" value={form.startDate} onChange={e => set("startDate", e.target.value)} /></Field><Field label="Chit value (₹) *"><input required type="number" min="1" value={form.chitValue} onChange={e => set("chitValue", e.target.value)} /></Field><Field label="Duration (months) *"><input required type="number" min="1" value={form.durationMonths} onChange={e => set("durationMonths", e.target.value)} /></Field><Field label="Member count *"><input required type="number" min="1" value={form.memberCount} onChange={e => set("memberCount", e.target.value)} /></Field><Field label="Base monthly contribution (₹) *"><input required type="number" min="0.01" step="0.01" value={form.installmentAmount} onChange={e => set("installmentAmount", e.target.value)} /></Field><Field label="Manager commission (%) *"><input required type="number" min="0" max="100" step=".01" value={form.commissionPercent} onChange={e => set("commissionPercent", e.target.value)} /></Field><Field label="Calculated manager fee"><input disabled value={money(commissionRupees)} /></Field><Field label="Initial lift amount (₹) *"><input required type="number" min="0" step="0.01" value={form.fixedInitialLiftAmount === "" && derivedInitial >= 0 ? derivedInitial : form.fixedInitialLiftAmount} onChange={e => set("fixedInitialLiftAmount", e.target.value)} /></Field><Field label="Monthly lift increment (₹) *"><input required type="number" min="0" step="0.01" value={form.fixedMonthlyIncrement} onChange={e => set("fixedMonthlyIncrement", e.target.value)} /></Field><Field label="Post-lift monthly payment"><input disabled value={money(Number(form.installmentAmount || 0) + Number(form.fixedMonthlyIncrement || 0))} /></Field><Field label="Late penalty per payment (₹)"><input type="number" min="0" value={form.latePenaltyAmount} onChange={e => set("latePenaltyAmount", e.target.value)} /></Field><Field label="Security deposit per member (₹)"><input type="number" min="0" value={form.securityDepositAmount} onChange={e => set("securityDepositAmount", e.target.value)} /></Field></div><p className="notice">Manager commission is a percent of chit value. Initial lift defaults to chit value minus that fee, but remains configurable. Each later month adds the configured increment. Post-lift monthly payment is base contribution plus that increment.</p>{error && <p className="red small">{error}</p>}<div className="row spacer"><Button type="button" onClick={onClose}>Cancel</Button><Button className="primary" disabled={busy} type="submit">{busy ? "Saving…" : submitLabel}</Button></div></form></Modal>;
}
...
function ChitPaymentModal({ token, installment, memberName, close, done }) {
...
function ChitMemberDetails({ token, scheme, enrollment, cycles, bids, installments = [], back, onPortalChange, onDeleted }) {
...
function FixedChitPaymentModal({ token, payment, memberName, close, done }) {
...
function FixedChitMemberDetails({ token, scheme, enrollment, lift, payments, back, recordPayment, deletePayment, onPortalChange, onDeleted }) {
...
function FixedChitSchemeDetails({ token, scheme, back, onSchemeDeleted }) {
  ...
  const openFixedPayment = item => setPayment({
    ...item,
    memberName: enrollmentName(data.enrollments.find(row => row.id === item.enrollment_id)),
  });
...
function PredefinedPaymentModal({ token, payment, memberName, close, done }) {
...
function PredefinedChitMemberDetails({ token, scheme, enrollment, item, payments, back, recordPayment, deletePayment, onPortalChange, onDeleted }) {
  const portalId = enrollmentPortalId(enrollment);
  const balance = payments.reduce((sum, row) => sum + Number(row.amount_due) - Number(row.amount_paid), 0);
  return <main className="shell"><div className="toolbar"><div><Button onClick={back}>← Members</Button><h1 className="title spacer">{enrollmentName(enrollment)}</h1><p className="copy">{scheme.name} · Ticket {enrollment.ticket_number} · Fixed Predefined Bid · User ID: {portalId || "Not enabled"}</p></div><div className="row"><ChitPortalAccess token={token} enrollment={enrollment} onChange={onPortalChange} /><ChitDeleteMemberControl token={token} scheme={scheme} enrollment={enrollment} onDeleted={onDeleted} /></div></div><div className="grid metrics"><Metric label="Lift month" value={item ? `Month ${item.month_number}` : "Not assigned"} color="blue" /><Metric label="EMI" value={item ? money(item.emi) : "—"} /><Metric label="Bid amount" value={item ? money(item.bid_amount) : "—"} color="gold" /><Metric label="Net receivable" value={item ? money(item.net_receivable) : "—"} color="green" /><Metric label="Outstanding" value={money(Math.max(0, balance))} color="red" /><Metric label="User ID" value={portalId || "Not enabled"} color={portalId ? "gold" : ""} /></div>{portalId && <p className="notice">Share this User ID with the member. Use Reset PIN to set the PIN they will use on Chit customer login, then share both privately.</p>}<div className="card spacer"><strong>Payment history</strong><div className="table spacer chit-member-payments"><table><thead><tr><th>Month</th><th>Due date</th><th>Payment date</th><th>Expected</th><th>Paid</th><th>Balance</th><th>Late fee</th><th>Reference</th><th>Status</th><th></th></tr></thead><tbody>{payments.map(row => <tr key={row.id}><td>Month {row.payment_month}</td><td>{formatChitDate(row.due_date)}</td><td>{formatChitDate(row.paid_date)}</td><td>{money(row.amount_due)}</td><td>{money(row.amount_paid)}</td><td className="red">{money(Math.max(0, Number(row.amount_due) - Number(row.amount_paid)))}</td><td>{row.late_penalty ? money(row.late_penalty) : "—"}</td><td>{row.payment_reference || "—"}</td><td><ChitPaymentStatus row={row} /></td><td><Button onClick={() => recordPayment(row)}>{row.amount_paid ? "Edit" : "Record"} Payment</Button>{Number(row.amount_paid) > 0 && <Button className="danger" onClick={() => deletePayment(row.id)}>Delete</Button>}</td></tr>)}</tbody></table>{!payments.length && <p className="small spacer">Payment rows appear after this scheme is activated.</p>}</div></div></main>;
}

function PredefinedBidSchemeDetails({ token, scheme, back, onSchemeDeleted }) {
  ...
  const visiblePredefinedPayments = paymentsByMemberName(data.predefinedPayments.filter(item => Number(item.payment_month) === selectedPaymentMonth), data.enrollments);
  const usedEnrollmentIds = new Set(completed.map(item => item.enrollment_id));
  const nextTicket = Math.max(0, ...data.enrollments.map(item => Number(item.ticket_number) || 0)) + 1;
  const openPredefinedPayment = item => setPayment({
    ...item,
    memberName: enrollmentName(data.enrollments.find(row => row.id === item.enrollment_id)),
  });
  const removePayment = async id => {
...
function AuctionChitSchemeDetails({ token, scheme, back, onSchemeDeleted }) {
  ...
  const openAuctionPayment = item => setPayment({
    ...item,
    memberName: enrollmentName(data.enrollments.find(row => row.id === item.enrollment_id)),
  });
  const calc = current ? {
  ...
  return <main className="shell">
    ...
    {payment && <ChitPaymentModal token={token} installment={payment} memberName={payment.memberName} close={() => setPayment(null)} done={async () => { setPayment(null); await refresh(); }} />}
  </main>;
}
...
function ChitSchemeCard({ kind, row, open, edit, activate, token, onDeleted }) {
  ...
  return <article className="chit-scheme-card" onClick={() => open(scheme)}>
    ...
    {scheme.status === "draft" && <div className="chit-scheme-actions" onClick={event => event.stopPropagation()}>
      ...
      <ChitDeleteSchemeControl token={token} scheme={scheme} memberCount={members.length} onDeleted={onDeleted} />
    </div>}
    {scheme.status !== "draft" && <div className="chit-scheme-actions" onClick={event => event.stopPropagation()}>
      <ChitDeleteSchemeControl token={token} scheme={scheme} memberCount={members.length} onDeleted={onDeleted} />
    </div>}
...
function ChitSchemeDashboardSection({ kind, rows, busy, open, edit, activate, token, onDeleted }) {
  const meta = CHIT_BOARD[kind];
  return <section className={`chit-board chit-board-${kind}`}>
    ...
    {rows.length ? <div className="chit-board-grid">{rows.map(row => <ChitSchemeCard key={row.scheme.id} kind={kind} row={row} open={open} edit={edit} activate={activate} token={token} onDeleted={onDeleted} />)}</div> : !busy && <p className="small chit-board-empty">No {meta.title} yet.</p>}
  </section>;
}

export function ChitFundPage({ token, close, openSchemeId = null, onOpenSchemeConsumed, onSchemesChanged }) {
  ...
  const editScheme = scheme => {
    setSchemeForm({
      ...
      commissionPercent: scheme.chit_type === CHIT_TYPES.FIXED
        ? fixedCommissionPercentFromAmount(scheme.chit_value, scheme.fixed_commission_amount)
  ...
  return <main className="shell chit-fund-page">
    ...
    <ChitSchemeDashboardSection kind="auction" rows={rows.filter(row => (row.scheme.chit_type || CHIT_TYPES.AUCTION) === CHIT_TYPES.AUCTION)} busy={busy} open={setSelected} edit={editScheme} activate={activate} token={token} onDeleted={schemeDeleted} />
    <ChitSchemeDashboardSection kind="fixed" rows={rows.filter(row => row.scheme.chit_type === CHIT_TYPES.FIXED)} busy={busy} open={setSelected} edit={editScheme} activate={activate} token={token} onDeleted={schemeDeleted} />
    <ChitSchemeDashboardSection kind="predefined" rows={rows.filter(row => row.scheme.chit_type === CHIT_TYPES.FIXED_PREDEFINED_BID)} busy={busy} open={setSelected} edit={editScheme} activate={activate} token={token} onDeleted={schemeDeleted} />
