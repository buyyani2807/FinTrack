...
function ChitMembersToolbar({ scheme, enrollments, onAddMember, token, changed }) {
...
function FixedChitMemberDetails({ token, scheme, enrollment, lift, payments, back, recordPayment, deletePayment, onPortalChange, onDeleted, orgSettings = {} }) {
...
function PredefinedChitMemberDetails({ token, scheme, enrollment, item, payments, back, recordPayment, deletePayment, onPortalChange, onDeleted, orgSettings = {} }) {
  ...
  if (showStatement) {
    return <CustomerStatementPage
      mode="chit"
      settings={orgSettings}
      chit={{ scheme, enrollment, payments, predefinedItem: item }}
      back={() => setShowStatement(false)}
    />;
  }
...
function PredefinedBidSchemeDetails({ token, scheme, back, onSchemeDeleted, orgSettings = {}, workspace = {}, onReceipt }) {
  ...
  if (member) {
    ...
    return <><PredefinedChitMemberDetails token={token} scheme={scheme} enrollment={member} item={assigned} payments={memberPaymentsForEnrollment(data.predefinedPayments, member.id)} back={() => setMember(null)} recordPayment={openPredefinedPayment} deletePayment={removePayment} orgSettings={orgSettings} onPortalChange={async () => { const payload = await loadChitSchemeDetails(token, scheme.id); setData(payload); setMember(payload.enrollments.find(item => item.id === member.id) || member); }} onDeleted={async removed => { setMember(null); setNotice(`${enrollmentName(removed)} was removed from ${scheme.name}.`); await refresh(); }} />{payment && <PredefinedPaymentModal token={token} payment={payment} memberName={payment.memberName} memberPhone={payment.memberPhone} scheme={scheme} close={() => setPayment(null)} done={async () => { setPayment(null); await refresh(); }} orgSettings={orgSettings} workspace={workspace} onReceipt={onReceipt} />}</>;
  }
  if (tab === "payments") {
