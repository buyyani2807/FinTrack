...
function AuctionChitSchemeDetails({ token, scheme, back }) {
  ...
  return <main className="shell">
    ...
    {busy ? <p className="small spacer">Loading scheme details…</p> : <>
      ...
      {tab === "payments" && <div className="card"><strong>Payments</strong><div className="table spacer"><table><thead><tr><th>Member</th><th>Ticket</th><th>Month due</th><th>Expected</th><th>Paid</th><th>Status</th><th></th></tr></thead><tbody>{paymentsByMemberName(data.installments, data.enrollments, data.cycles).map(item => { const owner = data.enrollments.find(row => row.id === item.enrollment_id); const cycle = data.cycles.find(row => row.id === item.cycle_id); return <tr key={item.id}><td>{enrollmentName(owner)}</td><td>{owner?.ticket_number || "—"}</td><td>{cycle ? `Month ${cycle.cycle_number}` : "—"}</td><td>{money(item.net_amount_due)}</td><td>{money(item.amount_paid)} {item.payment_mode || ""}</td><td>{item.status}</td><td><Button onClick={() => openAuctionPayment(item)}>{item.amount_paid ? "Edit payment" : "Record payment"}</Button>{item.amount_paid > 0 && <Button className="danger" onClick={() => remove(item.id)}>Delete</Button>}</td></tr>; })}</tbody></table>{!data.installments.length && <p className="small">Record a monthly bid first to create member installments.</p>}</div></div>}
